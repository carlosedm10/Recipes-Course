...
class PublicUserApiTests(TestCase):
    ...
    def test_create_token_for_user(self):
        ...
        self.assertIn('token', response.data) # We are checking that the token is in the response.
        self.assertEqual(response.status_code, status.HTTP_200_OK) # HTTP 200 response indicates that the request was successful.
    ...
    def test_create_token_invalid_credentials(self):
        ...
        self.assertNotIn('token', response.data) # We are checking that the token is not in the response.
...
class PrivateUserApiTests(TestCase):
    ...
    def setUp(self):
        ...
        self.client = APIClient()
        self.client.force_authenticate(user=self.user) # This lets us not use a real authentication.
